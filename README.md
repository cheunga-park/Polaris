...
| 3 물체·씬 구성 | 물체 2개 + GUI → 업스트림 검증 통과 폴더 | **부분 통과** 2026-09-09 — 합성 그릇 영상 → SAM 2 마스크 5장 → TRELLIS(자체 micromamba 환경) → 텍스처 GLB/USDZ(4,830면, 속이 빈 그릇). compose GUI 는 `/compose-environments/`. 남은 것: 실제 크기 적용(size_m), 업스트림 검증 스크립트 통과 확인 |
| 4 폰 업로드·문서 | 폰 브라우저에서 업로드 → 결과 | — |
| 5 MuJoCo 평가 | 업스트림 eval.py 무수정 실행, 환경별 50 롤아웃 | **S5.1–5.6 통과, 수치 맞추기 진행 중** 2026-09-14 — 업스트림 `eval.py` 무수정(import 후킹). π0.5 30회 기준 PanClean 진행도 0.66 / 성공 13 %(논문 0.55, Isaac 재현 0.75 / 37 %), FoodBussing 0.51 / 0 %(0.58 / 17 %), BlockStack 0.36 / 3 %(0.54 / 2 %). 델타·수정 기록은 PLAN.md §8.5, §10. 최종 50 롤아웃 스윕 `runs/pi05_final` 진행 중, 결과는 `/results.html` |
